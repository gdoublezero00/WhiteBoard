...
<paper-header-panel class="flex" mode="seamed">
	...
	<form is="iron-form" id="form" action="./upload-recv" method="post" enctype="multipart/form-data">
		...
		<paper-input type="text" name="name"></paper-input>
...
<script>
document.querySelector('[name=file]').addEventListener('change', function(event) {
	document.querySelector('[name=name]').value = event.target.value.split("\\").pop();
});
</script>
</body>
